...
export const calculateRecipeTotalCost = (
    ...
    // Calcular el costo de mano de obra por porción
    const laborCostPerServing = recipe.servings > 0 ? recipe.laborCost / recipe.servings : 0;
    
    // Calcular el subtotal (solo ingredientes) y luego agregar los gastos generales
    const subTotal = ingredientsCost * (1 + (recipe.overheadPercentage / 100));
    
    // Calcular el costo total por porción (ingredientes + gastos generales + mano de obra)
    const costPerServing = (recipe.servings > 0 ? subTotal / recipe.servings : 0) + laborCostPerServing;
    
    // Calcular el costo total (costo por porción multiplicado por el número de porciones)
